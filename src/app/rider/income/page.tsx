...
export default function RiderIncomePage() {
  ...
  const net = (d: any) => (d.deliveryFee || 0) * (1 - DELIVERY_CUT_RATE)
  ...
  const sumFor = (key: string) => deliveries.filter((d) => dateKey(d.createdAt) === key).reduce((s, d) => s + net(d), 0)

  const todayIncome = sumFor(todayKey)
  const yesterdayIncome = sumFor(yesterdayKey)
  const totalIncome = deliveries.reduce((s, d) => s + net(d), 0)
      ...
      <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '30px 20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
          <div>
            ...
            <p style={{ fontSize: '16px', color: 'gray' }}>After 20% platform cut per delivery</p>
          </div>
        ...
        <div style={{ backgroundColor: 'white', padding: '20px', borderRadius: '12px', border: '3px solid black', boxShadow: '4px 4px 0px black', marginBottom: '30px' }}>
          ...
          {selectedDate && (
            <div style={{ marginTop: '15px', padding: '15px', backgroundColor: '#fff3e0', borderRadius: '8px', border: '2px solid #ff9800', textAlign: 'center' }}>
              <p style={{ fontWeight: 'bold', margin: 0, fontSize: '18px' }}>💵 Net income for {selectedDate}: ₱{selectedIncome.toFixed(2)}</p>
            </div>
          )}
        </div>
            ...
            {displayList.map((d) => (
              <div key={d.id} style={{ backgroundColor: 'white', padding: '15px 20px', borderRadius: '12px', border: '3px solid black', boxShadow: '4px 4px 0px black', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
                <div>
                  ...
                  <p style={{ fontSize: '12px', color: 'gray', margin: 0 }}>{new Date(d.createdAt).toLocaleString()} | Fee ₱{(d.deliveryFee || 0).toFixed(2)}</p>
                </div>
                <p style={{ fontSize: '20px', fontWeight: 'bold', color: '#4caf50', margin: 0 }}>+₱{net(d).toFixed(2)}</p>
              </div>
            ))}
          </div>
        )}
