  ...
  return (
    <header style={{ backgroundColor: 'white', borderBottom: '3px solid black', position: 'sticky', top: 0, zIndex: 100 }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '15px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div onClick={() => router.push('/rider/dashboard')} style={{ fontSize: '20px', fontWeight: 'bold', cursor: 'pointer' }}>
          🏍️ Tindahan Rider
        </div>

        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
          <div className="desktop-nav" style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
            <button onClick={() => router.push('/rider/dashboard')} style={buttonStyle}>🏍️ Dashboard</button>
            <button onClick={() => router.push('/rider/orders')} style={buttonStyle}>📦 My Deliveries</button>
            <button onClick={() => router.push('/rider/income')} style={buttonStyle}>💰 Income</button>
      ...
      {menuOpen && (
        <div style={{ backgroundColor: 'white', borderTop: '2px solid black', padding: '15px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            <button onClick={() => { router.push('/rider/dashboard'); setMenuOpen(false) }} style={{ ...buttonStyle, textAlign: 'left', padding: '12px' }}>🏍️ Dashboard</button>
            <button onClick={() => { router.push('/rider/orders'); setMenuOpen(false) }} style={{ ...buttonStyle, textAlign: 'left', padding: '12px' }}>📦 My Deliveries</button>
            <button onClick={() => { router.push('/rider/income'); setMenuOpen(false) }} style={{ ...buttonStyle, textAlign: 'left', padding: '12px' }}>💰 Income</button>
